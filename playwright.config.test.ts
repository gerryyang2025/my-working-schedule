...
describe("Playwright config", () => {
  it("uses an isolated temporary data file for e2e API storage", () => {
    ...
    expect(apiServer?.env?.SCHEDULE_DATA_PATH).toEqual(expect.stringContaining(tmpdir()));
    expect(apiServer?.env?.SCHEDULE_DATA_PATH).not.toBe("data/e2e.local.json");
  });
