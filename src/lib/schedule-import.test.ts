...
describe("validateScheduleImportText", () => {
  ...
  it("parses Markdown tables generated by AI tools", () => {
    const markdownText = `当前排班周期为2026年7月20日（周一）至 7月26日（周日）：
...
| 张曼曼 | N1 | / | 休 | P3 | A4 | A4 | N1 |`;
    ...
    const preview = expectPreview(validateScheduleImportText({ rawText: markdownText, data: baseData() }));

    expect(preview.summary.importableCells).toBe(14);
    expect(preview.rows[0].staffName).toBe("段鸿露");
    expect(preview.rows[1].cells[1]).toMatchObject({
      rawValue: "/",
      shiftId: "shift-slash",
      status: "import"
