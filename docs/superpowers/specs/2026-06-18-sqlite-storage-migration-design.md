...
`tools/sqlite-service.sh` 是当前系统的 SQLite 数据库文件维护入口，不是 SQLite 后台服务进程。脚本面向 Linux 部署环境，封装安装检查、目录准备、迁移、备份、恢复和健康检查；Web/API 进程仍由 `optools`、systemd 或后续正式部署脚本独立管理。
...
| `restore <backup-file>` | 从指定备份文件恢复 SQLite 数据库，必须显式设置 `CONFIRM_RESTORE=yes`；恢复前必须先备份当前数据库，并确保 `SCHEDULE_BACKUP_PATH` 与 `SCHEDULE_SQLITE_PATH` 所在文件系统都有足够可用空间 |
| `status` | 显示 SQLite 数据库路径、是否存在、文件大小、最后修改时间、备份目录和当前配置 |
...
- `restore` 会清理目标 SQLite 文件旁的 `-journal`、`-wal`、`-shm` sidecar；若最终原子替换失败，需要把当前数据库及其 sidecar 回滚恢复。
...
1. 要求显式设置 `CONFIRM_RESTORE=yes`。
2. 支持传入绝对备份路径，或只传入 `SCHEDULE_BACKUP_PATH` 下的简单文件名。
3. 恢复前自动备份当前数据库。
4. 在 SQLite 目标目录创建同目录临时副本后再原子替换。
5. 清理目标库旁的 `-journal`、`-wal`、`-shm` sidecar；若最终替换失败，回滚恢复之前的数据库与 sidecar。

### `data:check:sqlite`

职责：

1. 执行 SQLite `integrity_check`。
2. 检查核心业务表是否存在。
3. 输出包含 `ok` 字段的结构化结果。
